...
import { getRooms, getReservations, createReservation, Room } from 'pages/remotes';
...
export function ReservableRoomList() {
  ...
  const { date, startTime, endTime } = reservationFilter;
      ...
      {availableRooms.length === 0 ? (
        ...
        >
          {availableRooms.map(room => (
            <ReservableRoomListItem key={room.id} room={room} />
          ))}
        </div>
      )}
      <Spacing size={16} />
...
function ReservableRoomListItem({ room }: { room: Room }) {
